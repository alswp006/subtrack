 ...
 *   SubTrack -> the app's display name
 *   OTT·앱 등 매달 자동결제되는 구독 서비스를 한곳에 모아 총 지출과 갱신일을 관리해주는 구독 트래커    -> the one-line description
 */
...
export default function Home() {
  ...
  return (
    <ScreenScaffold
      top={<Top title={<Top.TitleParagraph>SubTrack</Top.TitleParagraph>} />}
    >
      {/* 시각 앵커: 헤드라인 + 카드 내 진입 버튼(부유 금지, display="block" 전체폭).
          데이터 앱이면 value를 <Amount typography="t1" />(핵심 숫자)로 교체하라. */}
      <SummaryHero
        label="SubTrack"
        value={<Paragraph.Text typography="t2">OTT·앱 등 매달 자동결제되는 구독 서비스를 한곳에 모아 총 지출과 갱신일을 관리해주는 구독 트래커</Paragraph.Text>}
