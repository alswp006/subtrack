...
describe("해지 체크리스트 화면 완성 (0014 재작업, 스텁 대체) [packet heal-1-03]", () => {
  beforeEach(() => {
  ...
  describe("AC-1: 체크리스트 5개 항목이 ListRow + Switch로 렌더되고 토글 시 haptic + 즉시 저장 + 새로고침 유지", () => {
    it("AC-1[P0]: renders all 5 DEFAULT_CHECKLIST labels as switches, all unchecked initially", async () => {
      ...
      expect(DEFAULT_CHECKLIST.length).toBe(5);
  ...
  // ========== AC-4 ==========
  describe("AC-4: '해지 완료로 표시' 탭 시 status가 CANCELED로 바뀌고 상세로 replace 이동하며 대시보드 총액에서 제외된다", () => {
    it("AC-4[P0]: marks the subscription CANCELED, navigates (replace) to its detail screen with a toast, and excludes it from the active monthly total", async () => {
      seedLocalStorage({
      ...
      await waitFor(() => {
        expect(mockNavigate).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            replace: true,
            state: expect.objectContaining({ toast: "해지 완료로 표시했어요" }),
          }),
        );
      });
    ...
    it("AC-4[P0]: does not mark or navigate anywhere when the complete button has not been tapped", async () => {
      seedLocalStorage({
        [STORAGE_KEYS.subscriptions]: [makeSub({ id: "sub-1", amount: 9900, cycle: "MONTHLY" })],
      });
      ...
      expect(getStoredSub("sub-1")?.status).toBe("ACTIVE");
    });
  });

  // ========== AC-5 ==========
  describe("AC-5: 루트가 ScreenScaffold(screen-checklist)이고 렌더 시 console.error가 0건이다", () => {
    it("AC-5[P0]: root carries data-testid='screen-checklist' and renders with zero console.error calls", async () => {
