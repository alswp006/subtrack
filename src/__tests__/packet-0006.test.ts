...
describe("packet-0006: Settings Repository (getSettings/saveSettings/isCompareUnlocked/unlockCompare)", () => {
  ...
  describe("AC-1: getSettings returns default when localStorage is empty", () => {
    it("should return default AppSettings with expected shape when storage is empty", () => {
      const { getSettings } = require("@/domain/settings");
    ...
    it("should not throw exception and return null fields for timestamps", () => {
      const { getSettings } = require("@/domain/settings");
      const result = getSettings();
    ...
    it("should preserve existing settings from localStorage on second call", () => {
      const { getSettings, saveSettings } = require("@/domain/settings");
  ...
  describe("AC-2: isCompareUnlocked returns true if compareUnlockedAt is within 24 hours, false otherwise", () => {
    it("should return true when compareUnlockedAt is 23 hours ago", () => {
      const { saveSettings, isCompareUnlocked } = require("@/domain/settings");
    ...
    it("should return false when compareUnlockedAt is 25 hours ago", () => {
      const { saveSettings, isCompareUnlocked } = require("@/domain/settings");
    ...
    it("should return false when compareUnlockedAt is null", () => {
      const { getSettings, isCompareUnlocked } = require("@/domain/settings");
    ...
    it("should return true when compareUnlockedAt is exactly now (0ms ago)", () => {
      const { saveSettings, isCompareUnlocked } = require("@/domain/settings");
    ...
    it("should return true when compareUnlockedAt is 1 hour ago", () => {
      const { saveSettings, isCompareUnlocked } = require("@/domain/settings");
  ...
  describe("unlockCompare sets compareUnlockedAt to current timestamp", () => {
    it("should set compareUnlockedAt to current ISO timestamp", () => {
      const { unlockCompare, getSettings } = require("@/domain/settings");
    ...
    it("should make isCompareUnlocked return true immediately after unlockCompare", () => {
      const { unlockCompare, isCompareUnlocked } = require("@/domain/settings");
...
describe("packet-0006: Checklist Repository (getChecklist/toggleChecklistItem/getChecklistProgress)", () => {
  ...
  describe("AC-3: getChecklist returns DEFAULT_CHECKLIST with 5 items on first call", () => {
    it("should return a CancelChecklist with 5 items, all done:false", () => {
      const { getChecklist } = require("@/domain/checklists");
    ...
    it("should have ChecklistItem with id, label, done, doneAt properties", () => {
      const { getChecklist } = require("@/domain/checklists");
    ...
    it("should return a new copy on each call (not mutating stored default)", () => {
      const { getChecklist } = require("@/domain/checklists");
  ...
  describe("AC-3: toggleChecklistItem toggles done flag and sets doneAt", () => {
    it("should toggle first item from done:false to done:true with ISO doneAt", () => {
      const { getChecklist, toggleChecklistItem } = require("@/domain/checklists");
    ...
    it("should toggle item back to done:false and clear doneAt", () => {
      const { getChecklist, toggleChecklistItem } = require("@/domain/checklists");
    ...
    it("should persist toggles across different checklist IDs independently", () => {
      const { getChecklist, toggleChecklistItem } = require("@/domain/checklists");
  ...
  describe("AC-4: getChecklistProgress returns {done, total} counts", () => {
    it("should return {done:0,total:5} for new checklist", () => {
      const { getChecklist, getChecklistProgress } = require("@/domain/checklists");
    ...
    it("should return {done:2,total:5} after toggling 2 items", () => {
      const { getChecklist, toggleChecklistItem, getChecklistProgress } =
        require("@/domain/checklists");
    ...
    it("should return {done:5,total:5} when all items are complete", () => {
      const { getChecklist, toggleChecklistItem, getChecklistProgress } =
        require("@/domain/checklists");
    ...
    it("should return {done:1,total:5} after toggling one item on and one item off", () => {
      const { getChecklist, toggleChecklistItem, getChecklistProgress } =
        require("@/domain/checklists");
